...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createTable(data) {
        var container = document.getElementById('table-container-cc');
        container.innerHTML = '';
        ...
        fetch('get-edgelist-data-cc')
            ...
            .then(graph_data => {
                ...
                graph_data.nodes.forEach(node => {
                    ...
                    node.symbolSize = 20 + 30 * (node.value ); // 根据度值调整大小
                    node.label = {
                        normal: {
    ...
    // 事件监听器
    document.getElementById('closeness-btn').addEventListener('click', function() {
    ...
    function fetchGraphDatatop10(withWeight) {
        ...
        var url = 'get-edgelist-data-cc-top10?index=' + selectedNodeIndex;
        fetch(url)
            .then(response => response.json())
            .then(graph_data => {
                initChart2()
                ...
                // 为每个节点设置样式，前十的节点标红，其他节点标蓝
                graph_data.nodes.forEach(node => {
                    node.symbolSize = top1.includes(node) ? 80 : 20;
                    node.label = {
                        normal: {
    ...
    document.getElementById('closeness-topo-btn').addEventListener('click', function() {
