...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createTable(data) {
        var container = document.getElementById('table-container-dc');
        container.innerHTML = '';
    ...
    function fetchGraphData(withWeight) {
        fetch('get-edgelist-data-dc')
            .then(response => response.json())
            .then(graph_data => {
                var maxDegree = Math.max(...graph_data.nodes.map(node => node.value));
    ...
    // 事件监听器
    document.getElementById('degree-btn').addEventListener('click', function() {
    ...
    function fetchGraphDatatop10(withWeight) {
        ...
        var url = 'get-edgelist-data-dc-top10?index=' + selectedNodeIndex;
        ...
        fetch(url)
            ...
            .then(graph_data => {
                ...
                graph_data.nodes.forEach(node => {
                    ...
                    degree[node.name] = node.value;
                    var toponenode = top1.includes(node) ? node.name : null;
                });
                var option1 = {
                    series: [{
                        type: 'graph',
    ...
    document.getElementById('degree-topo-btn').addEventListener('click', function() {
